...
func (c *config) linkGo(fn string) (err error) {
	...
	header := fmt.Sprintf(pkgHeader, strings.Join(c.osArgs, " "), pkgName, crtPrefix, c.goos, c.goarch)
	...
	if l.Main {
		header = fmt.Sprintf(pkgHeader+mainHeader, strings.Join(c.osArgs, " "), "main", crtPrefix, c.goos, c.goarch) //TODO merge header constants
		crt0o, err := c.compile(crt.CRT0)
		if err != nil {
			return err
		}
