...
`
	mainHeader = `func main() { X_start(%[3]sMustNewTLS()) }
...
compilation terminated`, c.arg0)
	...
	sysPaths := []string{
		//TODO filepath.Join(libc, "arch", libcArch),
		//TODO filepath.Join(libc, "arch", "generic"),
...
func (c *config) linkGo(fn string) (err error) {
	...
	header := fmt.Sprintf(pkgHeader, strings.Join(c.osArgs, " "), pkgName, crtPrefix, c.goos, c.goarch, impCrt)
	...
	if l.Main {
		header = fmt.Sprintf(pkgHeader+mainHeader, strings.Join(c.osArgs, " "), "main", crtPrefix, c.goos, c.goarch, impCrt) //TODO merge header constants
		crt0o, err := c.compile(filepath.Join(crt.RepositoryPath, "crt0", "crt0.c"))
		if err != nil {
			return err
		}
